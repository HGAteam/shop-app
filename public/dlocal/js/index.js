const dlocalInstance = dlocal("ae32182e-a1a2-43b1-aa8f-bf6fb5e23991");
    ...
    resetButton.addEventListener("click", function (e) {
        ...
        if (installmentsInput) {
            ...
            installmentsInput.innerHTML =
                "<option style='color: #f0810f' value=''>-</option>";
        }
...
function registerEvents(exampleName, fields, fieldsNames) {
    ...
    fields.forEach(function (field, idx) {
        ...
        field.on("autofilled", function (event) {
            if (event.autofilled) {
                document
                    .getElementById(fieldsNames[idx])
                    .classList.add("autofilled");
            } else {
                document
                    .getElementById(fieldsNames[idx])
                    .classList.remove("autofilled");
            }
        });
...
function buildInstallments(installmentsInput, installmentsPlan) {
    const installmentsOptions = installmentsPlan.installments.reduce(function (
        options,
        plan
    ) {
        options = "<option value=" + plan.id + ">" + plan.installments + " of " + "USD" + " " + plan.installment_amount + " (Total : " + "USD" + " " + plan.total_amount + ")</option>";
        return options;
    },
    "");
